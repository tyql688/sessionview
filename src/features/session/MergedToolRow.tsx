...
export function MergedToolRow(props: {
    ...
    props.tools.length > 0
      ? props.tools
          .map((toolName, index) => {
            ...
            return `${toolIcon(toolName, metadata)} ${toolDisplayName(toolName, metadata)}`;
          })
          .join(", ")
      : "tools";
  ...
  return (
    <div className="merged-tools">
      <div className="merged-tools-header" onClick={() => setManualExpanded((v) => !v)}>
        <span className="merged-tools-label">{label}</span>
        <span className="merged-tools-chevron">{expanded ? "\u25BE" : "\u25B8"}</span>
      </div>
      {expanded && (
